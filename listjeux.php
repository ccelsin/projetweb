...
<body>
    <section class="home">
        <div class="container text-center">
            ...
            // Requête SELECT pour récupérer les jeux
            $result = $con->query("SELECT id, images, nom, categorie, FROM jeu");
            ...
            if ($result) {
                ...
                if ($result->num_rows > 0) {
                    ...
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo '<td>' . $row['categorie'] . '</td>';
                        echo '<td><a href="../projetweb/docpdf/' . $row['regle'] .'" download>Télécharger PDF</a></td>';
                        echo '<td><img src="../projetweb/images/' . $row['image'] . '" alt="Image du jeu"></td>';
                        echo '<td>
                                    <form method="POST" action="tt_like.php">
                                        <input type="hidden" name="game_id" value="' . $row['nom'] . '">
                                        <button class="btn btn-primary" type="submit">Like</button>
                                    </form>
                                </td>';
                        echo '<td> 
                                    <form method="POST" action="tt_Ins_Partie.php">
                                        <input type="hidden" name="game_id" value="' . $row['nom'] . '">
